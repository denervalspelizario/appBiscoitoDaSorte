...
class App extends Component {
  constructor(props){
    super(props);
    this.state ={   // 3 aonde fica os states
      textoFrase: '',  // 4 state inicial de texto no caso vazio
      img: ImageClose,
      textoBtn: 'Quebrar o Biscoito' 
    };

    this.quebraBiscoito = this.quebraBiscoito.bind(this);  // 5 bind para linkar ao state com a  funcao que mudanca o estado(setState)  

    this.frases = [   // 3 frases ao quebrar o biscoito da sorte
  ...
  quebraBiscoito(){  // 5 funcao que muda estado linkada ao state

    if(this.state.img === ImageOpen ){

      this.setState({
        textoFrase:  ' ' ,//  5 ao acionar o onPress o state vira setState e 1 - troca o textoFrase de vazio para frase alatoria atavez da funcao numeroAleatorio
        img: ImageClose,
  ...
  render(){
    return (
      <View style={styles.container}>
        <Image 
          source={this.state.img}  // 4 importando state image 
          style={styles.img}  // 1 estilizacao da imagem
        />
                                        {/* 4 state inicial de texto */}  
        <Text style={styles.textoFrase}>{this.state.textoFrase}</Text>  
        <TouchableOpacity style={styles.botao} onPress={this.quebraBiscoito}>{/* 2 Chamando o botão - 5  cria Onpress que ao pressionado chama state quebraBiscoito*/}
